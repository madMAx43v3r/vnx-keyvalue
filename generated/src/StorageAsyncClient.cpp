...
uint64_t StorageAsyncClient::get_version_key(const uint64_t& version, const std::function<void(::vnx::Variant)>& _callback, const std::function<void(const std::exception&)>& _error_callback) {
...
uint64_t StorageAsyncClient::get_version_keys(const std::vector<uint64_t>& versions, const std::function<void(std::vector<std::pair<uint64_t, ::vnx::Variant>>)>& _callback, const std::function<void(const std::exception&)>& _error_callback) {
...
void StorageAsyncClient::vnx_callback_switch(uint64_t _request_id, std::shared_ptr<const vnx::Value> _value) {
	...
	if(_type_hash == vnx::Hash64(0x4a92482e1381ab01ull)) {
		...
		if(!_result) {
			throw std::logic_error("StorageAsyncClient: !_result");
		}
		const auto _iter = vnx_queue_get_value.find(_request_id);
		if(_iter != vnx_queue_get_value.end()) {
